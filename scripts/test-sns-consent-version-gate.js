// test-sns-consent-version-gate.js
// sns-consent.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、sns-consent.html の <head> 最先頭ゲートが
...
//
// ★sns-consent は真の裸型（shared.js 非読込・外部JS皆無・sougei 型）。Option A
//   （ゲートのみ移植・bump非改修）。ゲート発火は version.txt 主導。
//
// 実行: node scripts/test-sns-consent-version-gate.js

const fs = require('fs');
const path = require('path');

const GENBA = fs.readFileSync(path.join(__dirname, '..', 'genba.html'), 'utf8');
const SNS = fs.readFileSync(path.join(__dirname, '..', 'sns-consent.html'), 'utf8');
...
// (1) sns-consent.html にゲートブロックが存在する（未移植なら extractGate が throw）
let snsGate = null;
try {
  snsGate = extractGate(SNS, 'sns-consent.html');
  pass++; console.log('  PASS sns-consent.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(snsGate, genbaGate, 'sns-consent のゲートブロックが genba と byte 単位で同一');

if (snsGate) {
  console.log('[sns-consent抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(snsGate);
  ...
  console.log('[sns-consent抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/sns-consent.html';
