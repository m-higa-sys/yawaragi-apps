// test-monitoring-version-gate.js
// monitoring.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、monitoring.html の <head> 最先頭ゲートが
...
//
// ★monitoring は shared.js を読む②勢だが Option A（ゲートのみ移植・bump非改修・
//   shared.js?v= は据置）。ゲート発火は version.txt 主導で shared.js?v= と独立。
//
// 実行: node scripts/test-monitoring-version-gate.js

const fs = require('fs');
const path = require('path');

const GENBA = fs.readFileSync(path.join(__dirname, '..', 'genba.html'), 'utf8');
const MONITORING = fs.readFileSync(path.join(__dirname, '..', 'monitoring.html'), 'utf8');
...
// (1) monitoring.html にゲートブロックが存在する（未移植なら extractGate が throw）
let monGate = null;
try {
  monGate = extractGate(MONITORING, 'monitoring.html');
  pass++; console.log('  PASS monitoring.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(monGate, genbaGate, 'monitoring のゲートブロックが genba と byte 単位で同一');

if (monGate) {
  console.log('[monitoring抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(monGate);
  ...
  console.log('[monitoring抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/monitoring.html';
