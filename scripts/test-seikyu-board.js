...
// 各フィクスチャは1回だけロード（recs4/recs5 は H/E セクションの既存束を再利用＝二重パース回避）
const recs3 = loadRecs('fixture-2026-03-kakuteimae.csv');
const recs6 = loadRecs('fixture-2026-06-hikiotoshimae.csv');
const t3 = tally(recs3);
const t4 = tally(recs4);
const t5 = tally(recs5);
const t6 = tally(recs6);

// 3月旧（全空欄・請求確定前）：pending=110 / exempt=5 / unpaid=0 / paid=0 / unknown=0 ← 空欄=pending の核心
ok(recs3.length === 115, 'I1: 3旧 個人115');
ok(t3.pending === 110 && t3.exempt === 5 && t3.unpaid === 0 && t3.paid === 0 && t3.unknown === 0,
   'I2: 3旧 pending110/exempt5/unpaid0/paid0/unknown0（誰も赤にならない）: ' + JSON.stringify(t3));

// 4月（引落し後）：unpaid=2 / exempt=5 / paid=111 / pending=0 / unknown=0
ok(recs4.length === 118, 'I3: 4月 個人118');
ok(t4.unpaid === 2 && t4.exempt === 5 && t4.paid === 111 && t4.pending === 0 && t4.unknown === 0,
   'I4: 4月 unpaid2/exempt5/paid111/pending0/unknown0: ' + JSON.stringify(t4));

// 5月（引落し後）：unpaid=2 / exempt=4 / paid=110 / pending=0 / unknown=0
ok(recs5.length === 116, 'I5: 5月 個人116');
ok(t5.unpaid === 2 && t5.exempt === 4 && t5.paid === 110 && t5.pending === 0 && t5.unknown === 0,
   'I6: 5月 unpaid2/exempt4/paid110/pending0/unknown0: ' + JSON.stringify(t5));

// 6月（請求確定済・引落し前・全未入金）：unpaid=106 / exempt=5 / pending=0 / paid=0 / unknown=0
ok(recs6.length === 111, 'I7: 6月 個人111');
ok(t6.unpaid === 106 && t6.exempt === 5 && t6.pending === 0 && t6.paid === 0 && t6.unknown === 0,
   'I8: 6月 unpaid106/exempt5/pending0/paid0/unknown0: ' + JSON.stringify(t6));
