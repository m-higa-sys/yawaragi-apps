// test-schedgrid-version-gate.js
// sched-grid.html の <head> インラインゲートに定義された純関数
...
// 実行: node scripts/test-schedgrid-version-gate.js

const fs = require('fs');
const path = require('path');

const SCHEDGRID_PATH = path.join(__dirname, '..', 'sched-grid.html');
const GENBA_PATH = path.join(__dirname, '..', 'genba.html');
const schedgridHtml = fs.readFileSync(SCHEDGRID_PATH, 'utf8');
...
const sandbox = {};
const src = extractFn(schedgridHtml, 'gateShouldReload', 'sched-grid.html') + '\n' +
  extractFn(schedgridHtml, 'buildVersionedUrl', 'sched-grid.html') +
...
const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/sched-grid.html';
...
eq(nrm(extractFn(schedgridHtml, 'gateShouldReload', 'sched-grid.html')),
   nrm(extractFn(genbaHtml, 'gateShouldReload', 'genba.html')), 'gateShouldReload が genba と一致');
eq(nrm(extractFn(schedgridHtml, 'buildVersionedUrl', 'sched-grid.html')),
