// test-oral-version-gate.js
// oral.html の <head> インラインゲートに定義された純関数
...
// 実行: node scripts/test-oral-version-gate.js

const fs = require('fs');
const path = require('path');

const ORAL_PATH = path.join(__dirname, '..', 'oral.html');
const GENBA_PATH = path.join(__dirname, '..', 'genba.html');
const oralHtml = fs.readFileSync(ORAL_PATH, 'utf8');
...
const sandbox = {};
const src = extractFn(oralHtml, 'gateShouldReload', 'oral.html') + '\n' +
  extractFn(oralHtml, 'buildVersionedUrl', 'oral.html') +
...
const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/oral.html';
...
eq(nrm(extractFn(oralHtml, 'gateShouldReload', 'oral.html')),
   nrm(extractFn(genbaHtml, 'gateShouldReload', 'genba.html')), 'gateShouldReload が genba と一致');
eq(nrm(extractFn(oralHtml, 'buildVersionedUrl', 'oral.html')),
