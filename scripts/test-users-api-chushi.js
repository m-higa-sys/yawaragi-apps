...
//   - 突合キーは「利用者名」の完全一致（台帳に「利用者008」「利用者009」等の別人が実在するため）。
...
const basic = build([
  ...
  ['2026-07-07', '2026-07-07', '2026-07-03', '利用者113', '家族都合'],
]);
eq(basic.map['利用者112'], '2026-06-30', '利用者112 の最終利用日を引ける');
eq(basic.map['利用者113'], '2026-07-07', '利用者113 の最終利用日を引ける');
...
const withEmpty = build([
  HEADER,
  ['', '', '2026-05-01', '空欄太郎', ''],
  ['2026-06-30', '2026-06-30', '2026-07-01', '利用者112', ''],
]);
...
eq(build([HEADER, ['2026-06-30', '', '', ' 利用者112 ', '']]).map['利用者112'], '2026-06-30', '利用者名の前後空白を trim');

// 列順は名前で探す（列が動いても壊れない）
const swapped = build([
  ['利用者名', '理由', '最終利用日'],
  ['利用者112', '本人意思', '2026-06-30'],
]);
eq(swapped.map['利用者112'], '2026-06-30', 'ヘッダ名で列を探す（列順非依存）');
...
const ledger = ['利用者112', '再開次郎', '利用者008', '利用者009'];
const d = diag(idx, ledger);
eq(d.unmatched, ['退所済子'], '台帳に居ない中止履歴行を全件報告');
eq(d.duplicates, [{ name: '再開次郎', count: 2 }], '同名複数行を全件報告');

// 別人の類似名は完全一致なので混ざらない
const idx2 = build([HEADER, ['2026-06-30', '', '', '利用者009', '']]);
const d2 = diag(idx2, ['利用者008', '利用者009']);
eq(d2.unmatched, [], '「利用者009」は完全一致で解決（未マッチ0）');
eq(idx2.map['利用者008'], undefined, '「利用者008」（別人）には波及しない');
