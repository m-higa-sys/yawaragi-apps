// test-cmsoufu-version-gate.js
// ケアマネ送付チェックリスト.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、ケアマネ送付チェックリストHTML の <head> 最先頭ゲートが
...
//
// ★本アプリは shared.js を読む②勢だが Option A（ゲートのみ移植・bump非改修・
//   shared.js?v= は据置）。ゲート発火は version.txt 主導で shared.js?v= と独立。
//
// 実行: node scripts/test-cmsoufu-version-gate.js
...
// (1) 対象HTML にゲートブロックが存在する（未移植なら extractGate が throw）
let cmGate = null;
try {
  cmGate = extractGate(CM, CM_FILE);
  pass++; console.log('  PASS ' + CM_FILE + ' にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(cmGate, genbaGate, 'ケアマネ送付チェックリストのゲートブロックが genba と byte 単位で同一');

if (cmGate) {
  console.log('[抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(cmGate);
  ...
  console.log('[抽出 buildVersionedUrl]');
  // buildVersionedUrl は genba と byte 同一の純関数（テスト2で保証済）＝ファイル名非依存。
  // 対象は日本語ファイル名で new URL() が path を percent-encode するため、純関数の
  // 挙動検証は他アプリ同様 ASCII ベースURLで行う（実挙動は正しく percent-encode される）。
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/cmsoufu.html';
