// test-cleaning-version-gate.js
// cleaning.html の <head> インラインゲートに定義された純関数
...
// 実行: node scripts/test-cleaning-version-gate.js

const fs = require('fs');
const path = require('path');

const CLEANING_PATH = path.join(__dirname, '..', 'cleaning.html');
const GENBA_PATH = path.join(__dirname, '..', 'genba.html');
const cleaningHtml = fs.readFileSync(CLEANING_PATH, 'utf8');
...
const sandbox = {};
const src = extractFn(cleaningHtml, 'gateShouldReload', 'cleaning.html') + '\n' +
  extractFn(cleaningHtml, 'buildVersionedUrl', 'cleaning.html') +
...
const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/cleaning.html';
...
eq(nrm(extractFn(cleaningHtml, 'gateShouldReload', 'cleaning.html')),
   nrm(extractFn(genbaHtml, 'gateShouldReload', 'genba.html')), 'gateShouldReload が genba と一致');
eq(nrm(extractFn(cleaningHtml, 'buildVersionedUrl', 'cleaning.html')),
