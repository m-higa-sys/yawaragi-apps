// test-height-version-gate.js
// height.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、height.html の <head> 最先頭ゲートが
...
//
// 実行: node scripts/test-height-version-gate.js

const fs = require('fs');
const path = require('path');

const GENBA = fs.readFileSync(path.join(__dirname, '..', 'genba.html'), 'utf8');
const HEIGHT = fs.readFileSync(path.join(__dirname, '..', 'height.html'), 'utf8');
...
// (1) height.html にゲートブロックが存在する（未移植なら extractGate が throw）
let heightGate = null;
try {
  heightGate = extractGate(HEIGHT, 'height.html');
  pass++; console.log('  PASS height.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(heightGate, genbaGate, 'height のゲートブロックが genba と byte 単位で同一');

if (heightGate) {
  console.log('[height抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(heightGate);
  ...
  console.log('[height抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/height.html';
