// test-weight-version-gate.js
// weight.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、weight.html の <head> 最先頭ゲートが
...
//
// 実行: node scripts/test-weight-version-gate.js

const fs = require('fs');
const path = require('path');

const GENBA = fs.readFileSync(path.join(__dirname, '..', 'genba.html'), 'utf8');
const WEIGHT = fs.readFileSync(path.join(__dirname, '..', 'weight.html'), 'utf8');
...
// (1) weight.html にゲートブロックが存在する（未移植なら extractGate が throw）
let weightGate = null;
try {
  weightGate = extractGate(WEIGHT, 'weight.html');
  pass++; console.log('  PASS weight.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(weightGate, genbaGate, 'weight のゲートブロックが genba と byte 単位で同一');

if (weightGate) {
  console.log('[weight抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(weightGate);
  ...
  console.log('[weight抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/weight.html';
