// test-sougei-version-gate.js
// sougei.html の <head> インラインゲートに定義された純関数
...
// 実行: node scripts/test-sougei-version-gate.js

const fs = require('fs');
const path = require('path');

const SOUGEI_PATH = path.join(__dirname, '..', 'sougei.html');
const GENBA_PATH = path.join(__dirname, '..', 'genba.html');
const sougeiHtml = fs.readFileSync(SOUGEI_PATH, 'utf8');
...
const sandbox = {};
const src = extractFn(sougeiHtml, 'gateShouldReload', 'sougei.html') + '\n' +
  extractFn(sougeiHtml, 'buildVersionedUrl', 'sougei.html') +
...
const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/sougei.html';
...
eq(nrm(extractFn(sougeiHtml, 'gateShouldReload', 'sougei.html')),
   nrm(extractFn(genbaHtml, 'gateShouldReload', 'genba.html')), 'gateShouldReload が genba と一致');
eq(nrm(extractFn(sougeiHtml, 'buildVersionedUrl', 'sougei.html')),
