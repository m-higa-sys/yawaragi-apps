...
// 郵送必着ロジックは furikae-mailing-core.js に委譲（furikae.html は <script src> で読み込む）。
// テストでは同一 core を require し、抽出関数(guessExpectedDate ラッパ/fnkCardHtml)の eval スコープへ注入する。
const FurikaeMailing = require('../furikae-mailing-core.js');
...
const scopeCard = {};
new Function('sb', 'FurikaeMailing',
  extractFn('fnkExtractResultCode') + '\n' +
  extractFn('fnkBadgeFor') + '\n' +
  extractFn('fnkTodoText') + '\n' +
  ...
  extractFn('fnkStartGuide') + '\n' +
  extractFn('fnkMd') + '\n' +
  extractFn('fnkCardTotal') + '\n' +
  'function fubiHistoryBadge(){return "";}\n' +
  extractFn('fnkCardHtml') + '\n' +
  'sb.fnkCardHtml = fnkCardHtml; sb.fnkCardTotal = fnkCardTotal; sb.fnkStartGuide = fnkStartGuide;'
)(scopeCard, FurikaeMailing);
...
ok(cardB.indexOf('までに投函') >= 0, 'code4→「M/D までに投函」（郵送でなく投函・必着ベース）');
ok(cardB.indexOf('電算に必着') >= 0, '★code4→「電算に必着」を表示（当社=yawaragi誤読を回避）');
ok(cardB.indexOf('当社') < 0, 'code4カード表示に「当社」を出さない（yawaragiと誤読される語）');
ok(cardB.indexOf('までに郵送') < 0, 'code4→旧「までに郵送」は消える（郵送=到着の誤り解消）');
...
// ===== (b) 郵送日記録→引落開始確定表示（formSentDate流用・fnkStartGuide純関数）=====
const fnkStartGuide = scopeCard.fnkStartGuide;
console.log('\n[fnkStartGuide 純関数]');
const sgP = fnkStartGuide({ resultCode: '4' }, '2026-06-01');
eq(sgP.mode, 'predict', 'code4・未記録→予告モード');
eq(sgP.furikaeDate, '2026-07-27', '予告：今日基準の次の振替日');
const sgS = fnkStartGuide({ resultCode: '4', formSentDate: '2026-07-11' }, '2026-06-01');
eq(sgS.mode, 'sent', 'code4・記録済→確定モード');
eq(sgS.expectedDate, '2026-08-27', 'sentDate=7/11→開始8/27（guessExpectedDate流用）');
eq(fnkStartGuide({ resultCode: '4', formSentDate: '2026-07-22' }, '2026-06-01').expectedDate, '2026-09-28', '★必着日7/22に投函→到着7/27→8/27に乗らず次サイクル9/28（輸送日数を乗せる新挙動）');
eq(fnkStartGuide({ resultCode: '4', formSentDate: '2026-07-23' }, '2026-06-01').expectedDate, '2026-09-28', '締切翌日7/23投函→到着7/28→9/28');
ok(fnkStartGuide({ resultCode: '2' }, '2026-06-01') === null, 'code2→ガイドなし(null)');
ok(fnkStartGuide({ resultCode: '1' }, '2026-06-01') === null, 'code1→ガイドなし(null)');
eq(fnkStartGuide({ resultCode: '4' }, '2026-06-01').mode, 'predict', 'formSentDate無→予告(後方互換)');
