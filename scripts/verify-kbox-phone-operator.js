...
okSafe(() => {
  const env = buildEnv();
  const api = bind(env);
  api.kbMarkPhoneDone('根岸君男', '2026-07-08');
  const h = env.__els['kbox-phone-operators'].innerHTML;
...
okSafe(() => {
  ...
  return !!t && t.name === '根岸君男' && t.date === '2026-07-08';
...
okSafe(() => {
  ...
  const api = bind(env);
  api.kbMarkPhoneDone('根岸君男', '2026-07-07');
...
okSafe(() => {
  ...
  api.kbConfirmPhoneDone_();
  const rec = env.__calls.filter(c => c.action === 'updateAbsenceCmNotified');
  return rec.length === 1 && rec[0].body.name === '根岸君男' && rec[0].body.date === '2026-07-08'
