...
// 評価順: 1.対象外(n=0/rate null/非有限) 2.データ不足(n<6) 3.保留(未分類件数>0)
//         4.減らし(<70) 5.増やし(>110) 6.適正圏だが低契約×率天井継続→増やし(保険) 7.それ以外=適正
...
{
  const results = ['対象外', 'データ不足', '保留', '減らし', '適正', '増やし'];
  ok(results.indexOf(J(null, 0)) !== -1, 'T3e: 返り値は既定6値のいずれか（NaN/Infinity/"0%"を返さない）');
  ok(results.indexOf(J(Infinity, 10)) !== -1, 'T3f: 非有限入力でも返り値は既定6値のいずれか');
}
