...
// --- IMPORTANT_DOMAINS 追加（2026-07-13 実弾dryRun反映）: ジョブメドレー ---
eqJson(classify('ジョブメドレー <system@job-medley.com>', 'メッセージが届きました'),
  { important: true, matchedBy: ['domain:job-medley.com'], excluded: false }, 'job-medley.com 一致（応募/メッセージ/面接すべてimportant）');
...
// --- EXCLUDE 追加（2026-07-13 実弾dryRun反映）: DAZN / LIXILでんき ---
eqJson(classify('DAZN <news@mail.dazn.com>', '今週の見どころ'),
  { important: false, matchedBy: [], excluded: true }, 'DAZN(mail.dazn.com) 除外');
eqJson(classify('LIXILでんき <ltspmypage@lixiltepco-sp.co.jp>', 'マイページのお知らせ'),
  { important: false, matchedBy: [], excluded: true }, 'LIXILでんき(lixiltepco-sp.co.jp) 除外');
...
// ===== nmSortItems_（2026-07-14追加・morningDigest Phase1）: メタデータ配列を仕分け＋id付与 =====
// checkNewMail のメッセージ→要素マッピングを純関数化。各要素に Gmail messageId を id として付与。
// 重複排除はしない（id違いは別メールso全部残す。住信SBIの別口座2通を消さないための核心設計）。
console.log('[nmSortItems_]');
const sort = F.nmSortItems_;

// id が important / others 全要素に付与される
const r1 = sort([
  { id: 'm1', from: '<x@carezou.net>', subject: 'お知らせ', date: '2026-07-14 09:00' },
  { id: 'm2', from: '友人 <f@example.com>', subject: 'ランチ', date: '2026-07-14 10:00' }
]);
eq(r1.important.length, 1, 'important 1件');
eq(r1.important[0].id, 'm1', 'important要素に id 付与');
eqJson(r1.important[0], { id: 'm1', from: '<x@carezou.net>', subject: 'お知らせ', date: '2026-07-14 09:00', matchedBy: ['domain:carezou.net'] }, 'important要素の形（id+matchedBy）');
eq(r1.others.count, 1, 'others 1件');
eq(r1.others.items[0].id, 'm2', 'others要素に id 付与');
eqJson(r1.others.items[0], { id: 'm2', from: '友人 <f@example.com>', subject: 'ランチ', date: '2026-07-14 10:00' }, 'others要素の形（id・matchedByなし）');

// ★核心: 住信SBI ― from/subject/date が完全一致でも id 違いなら両方残る
const r2 = sort([
  { id: 'sbi-A', from: '"住信SBIネット銀行" <post_master@netbk.co.jp>', subject: '定額自動振込サービス振込受付のお知らせ', date: '2026-07-14 02:34' },
  { id: 'sbi-B', from: '"住信SBIネット銀行" <post_master@netbk.co.jp>', subject: '定額自動振込サービス振込受付のお知らせ', date: '2026-07-14 02:34' }
]);
eq(r2.others.count, 2, '★住信SBI: from/subject/date同一でも id 違いなら両方残る（見落とし防止の核心）');
eq(r2.others.items[0].id + ',' + r2.others.items[1].id, 'sbi-A,sbi-B', '両方の id が保持される');

// EXCLUDE系（広告ドメイン / 自作アプリ通知プレフィックス）は id があっても除外
const r3 = sort([
  { id: 'ad1', from: 'ZOZO <x@zozo.jp>', subject: 'セール', date: '2026-07-14 08:00' },
  { id: 'yaw1', from: 'やわらぎ <x@keepfitlife.com>', subject: '【yawaragi】お休み連絡', date: '2026-07-14 08:00' }
]);
eq(r3.important.length, 0, 'EXCLUDE系は除外（important 0）');
eq(r3.others.count, 0, 'EXCLUDE系は除外（others 0）');
