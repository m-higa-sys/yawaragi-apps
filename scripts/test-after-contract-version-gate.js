// test-after-contract-version-gate.js
// after-contract.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、after-contract.html の <head> 最先頭ゲートが
...
//
// ★after-contract は真の裸型（shared.js 非読込・外部JS皆無・sougei 型）。Option A
//   （ゲートのみ移植・bump非改修）。ゲート発火は version.txt 主導。
//
// 実行: node scripts/test-after-contract-version-gate.js
...
// (1) after-contract.html にゲートブロックが存在する（未移植なら extractGate が throw）
let acGate = null;
try {
  acGate = extractGate(AC, 'after-contract.html');
  pass++; console.log('  PASS after-contract.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(acGate, genbaGate, 'after-contract のゲートブロックが genba と byte 単位で同一');

if (acGate) {
  console.log('[after-contract抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(acGate);
  ...
  console.log('[after-contract抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/after-contract.html';
