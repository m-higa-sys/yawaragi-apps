// test-tairyoku-version-gate.js
// tairyoku.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、tairyoku.html の <head> 最先頭ゲートが
...
//
// 実行: node scripts/test-tairyoku-version-gate.js

const fs = require('fs');
const path = require('path');

const GENBA = fs.readFileSync(path.join(__dirname, '..', 'genba.html'), 'utf8');
const TAIRYOKU = fs.readFileSync(path.join(__dirname, '..', 'tairyoku.html'), 'utf8');
...
// (1) tairyoku.html にゲートブロックが存在する（未移植なら extractGate が throw）
let tairyokuGate = null;
try {
  tairyokuGate = extractGate(TAIRYOKU, 'tairyoku.html');
  pass++; console.log('  PASS tairyoku.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(tairyokuGate, genbaGate, 'tairyoku のゲートブロックが genba と byte 単位で同一');

if (tairyokuGate) {
  console.log('[tairyoku抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(tairyokuGate);
  ...
  console.log('[tairyoku抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/tairyoku.html';
