// test-portal-version-gate.js
// portal.html の <head> インラインゲートに定義された純関数
...
// 実行: node scripts/test-portal-version-gate.js

const fs = require('fs');
const path = require('path');

const PORTAL_PATH = path.join(__dirname, '..', 'portal.html');
...
const sandbox = {};
const src = extractFn(portalHtml, 'gateShouldReload', 'portal.html') + '\n' +
  extractFn(portalHtml, 'buildVersionedUrl', 'portal.html') +
...
const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/portal.html';
...
eq(nrm(extractFn(portalHtml, 'gateShouldReload', 'portal.html')),
   nrm(extractFn(genbaHtml, 'gateShouldReload', 'genba.html')), 'gateShouldReload が genba と一致');
eq(nrm(extractFn(portalHtml, 'buildVersionedUrl', 'portal.html')),
