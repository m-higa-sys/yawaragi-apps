...
run('appData.users = ' + JSON.stringify([
  { name: '利用者110', furigana: 'リヨウシャ110', days: '月水', ampm: '午前' },
...
ok(tbodyHas('利用者110'), '現役: 利用者110が表示される');
...
ok(!tbodyHas('利用者110'), '検索ふるた: 現役はヒットしない');
...
run('appData.weights = ' + JSON.stringify({
  2026: { '利用者110': { '7月': 83.8 }, '田中太郎': { '6月': 60 }, '古田花子': { '5月': 48 }, '法浦武治': { '7月': 70 } }
}) + ';');
lastPost = null;
run('forceCloudSync();');
ok(lastPost !== null, '全件送信でPOSTが飛ぶ');
const sentW = lastPost.body.data.weights[2026];
ok(!!sentW['利用者110'], '送信: 現役(柳浦)は含む');
...
ok(!!lastPost.body.data.weights[2026]['利用者110'], '自動同期で現役(柳浦)は送信');
...
run('appData.weights = ' + JSON.stringify({ 2026: { '既存太郎': { '7月': 55 }, '利用者110': { '7月': 83.8 } } }) + ';');
...
window[cb9a]({ users: [
  { name: '利用者110', kana: 'リヨウシャ110', status: '', days: '月水', ampm: '午前' },
...
window[cb9d]({ users: [{ name: '利用者110', kana: 'リヨウシャ110', status: '' }] });
