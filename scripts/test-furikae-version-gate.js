// test-furikae-version-gate.js
// furikae.html のキャッシュ自動更新バージョンゲート横展開テスト（design.md §7 準拠）
//
// 方式：genba.html を正本とし、furikae.html の <head> 最先頭ゲートが
...
//
// 実行: node scripts/test-furikae-version-gate.js

const fs = require('fs');
const path = require('path');

const GENBA = fs.readFileSync(path.join(__dirname, '..', 'genba.html'), 'utf8');
const FURIKAE = fs.readFileSync(path.join(__dirname, '..', 'furikae.html'), 'utf8');
...
// (1) furikae.html にゲートブロックが存在する（未移植なら extractGate が throw）
let furikaeGate = null;
try {
  furikaeGate = extractGate(FURIKAE, 'furikae.html');
  pass++; console.log('  PASS furikae.html にゲート <script> ブロックが存在する');
} catch (e) {
  fail++; console.log('  FAIL ' + e.message);
}
// (2) genba と byte 単位で同一（verbatim 移植）
eq(furikaeGate, genbaGate, 'furikae のゲートブロックが genba と byte 単位で同一');

if (furikaeGate) {
  console.log('[furikae抽出 gateShouldReload]');
  const { gateShouldReload, buildVersionedUrl } = loadFns(furikaeGate);
  ...
  console.log('[furikae抽出 buildVersionedUrl]');
  const BASE = 'https://m-higa-sys.github.io/yawaragi-apps/furikae.html';
