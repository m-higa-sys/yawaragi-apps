...
ok(body.indexOf('【相談員】長期休み中の方への月1連絡のお願い') === 0, 'G1: 見出しは役割名（【相談員】）で始まる');
...
// 2026-08-01: 担当者が変わっても文面を直さずに済むよう、スタッフ個人名を本文から外す。
ok(body.indexOf('・担当者 → ご自身の名前を押す') !== -1, 'G10: 担当者の指示は役割ベース');
ok(body.indexOf('勝又') === -1, 'G11: スタッフ個人名を本文に含めない（担当交代で文面を直さないため）');
// 利用者名は業務上必須なので、G11 が氏名まで消していないことを固定する
ok(body.indexOf('甲野 一郎') !== -1, 'G12: 利用者名は残る（消しすぎ防止）');
