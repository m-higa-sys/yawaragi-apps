...
- [ ] **Step 7: 本番スモークテスト（実データ1件・所要30分・社長）**

> **必須の理由**: 全書込が `no-cors` である以上、成功表示は成否を意味しない。コードが本番に乗ったかを Read 直読で確認するだけでは「動くか」の検証にならない。**最悪シナリオ＝登録できたように見えて書き込まれていない（シートが空）**＝今回の事故（禁忌が見えず実施）と同じ結果。実データを1件通す以外に検証手段がない。**①〜⑧が全て通るまでスタッフへの周知は行わない。**

```
① kinki.html を開く
② 実在利用者に ⚠️caution を1件登録
   label:「テスト登録（後で解除）」／targetEquipment: WB など任意
③ 「禁忌」シートに実際に行が生成されたか目視確認
   ★ no-cors の成功表示は信用しない。シートを直接見ること
④ session-board.html を開き、氏名横にバッジが出るか確認
⑤ genba.html の導線リンク → 機器別ビューに飛ぶか確認
⑥ そのレコードを解除する（★登録だけで終えない・最初の1件目で releaseKinki 不具合が出る）
   - 2段確認モーダル（自前・confirm()でない）が正しく出るか
   - 解除理由プリセット・指示元入力（必須）が機能するか
   - 解除後、セッションボードのバッジが消えるか
⑦ 履歴に status=released として残っているか確認（★物理削除されていないこと）
⑧ D7検証（推奨・実装で最も「静かに壊れる」箇所＝氏名join採用の要）
   台帳に存在しない氏名で1件登録
   → セッションボード上部に「⚠️ 突合できない禁忌が1件あります」赤字警告が出るか
   → 確認後、そのレコードは解除して片付ける
```

- [ ] **Step 8: 完了メモリ更新**

`MEMORY.md` の進行中に禁忌P1の状態行を更新（本番反映SHA・版・アプリ台帳シート行追加済み・スモークテスト①〜⑧結果を証跡として記録）。全通過を確認してからスタッフ周知。P2（見直しリマインド＋伝達投稿）を宿題として残す。
...
## 実装順まとめ
Task1（コアTDD）→ Task2（GAS配線）→ Task3（kinki.html）→ Task4（ボードバッジ）→ Task5（genba導線）→ Task6（リリース：社長承認push＋verify＋**本番スモークテスト①〜⑧**）。
Task1〜5はworktree `C:/tmp/wt-kinki` 上で完結。Task6の本番書込＋スモークテストのみ社長の手。**スモークテスト①〜⑧が全通するまでスタッフ周知しない。**
