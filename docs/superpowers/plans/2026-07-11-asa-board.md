...
input.kaigoUsers             : { name, category, planStart:'YYYY-MM', planMonths:1-12, cancelled? }[]
    ← getKeikakushoTargetUsers_()（既定・cancelledは源で除外済み。includeCancelled=true は渡さない）
...
input.shienUsers             : { name, care }[]   ← care は category を写像（§2.2「要支援の care」規約）
...
出力: { date, year, month, presentCount, sokutei[], koukuMoni[], koukuTaisou[], kotan[], birthday[], residue[] }
  sokutei = 要介護(track:'kaigo')行 → 要支援(track:'shien')行 の順で連結（各系統内は remaining 昇順）。
  フロントは track で2系統に分離して各々を上位Nスライスする（§2.2）。
```
