...
| D10 | **解除権限は全スタッフ可**（ロール制限を導入しない）。安全は**証跡**（解除理由プリセット＋指示元必須＋操作者記録）で担保。ただし解除実行時に**確認モーダルを1枚**挟む | ロールは他アプリに無い概念so持ち込まない。誤タップ防止は確認1枚で足りる |
...
### 5.5 kinki.html：解除画面
**手順は「確認モーダル → 入力画面」の2段**（D10・誤タップ防止）。

**(1) 確認モーダル（自前実装・ブラウザ標準 `confirm()` は使わない＝文言を制御できないため）**
- 対象レコードの `label` を**動的表示**、`level` に応じて 🚫/⚠️ アイコンを切替。
- 「解除後、セッションボードのバッジは消えます」と明記。
- `[ キャンセル ]` / `[ 解除する ]` の2択。キャンセルで詳細に戻る。
```
┌──────────────────────────────┐
│ この禁忌を解除します。           │
│ 🚫 右膝 深屈曲NG   ← labelを動的表示│
│ 解除後、セッションボードのバッジは消えます。│
│        [ キャンセル ]  [ 解除する ] │
└──────────────────────────────┘
```

**(2) 入力画面（確認モーダルで [解除する] を押した後に進む）**
1. 解除理由（プリセット）
2. 補足（`その他` 選択時のみ必須）
3. **解除の指示元**（氏名・続柄・口頭/文書）…必須
4. 解除日（既定=今日）／解除操作者（記録）

登録時と同じ粒度で残す。**権限による制限はしない（全スタッフ可）。証跡で担保する（D10）。**
